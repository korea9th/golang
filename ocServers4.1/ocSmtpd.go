// Package smtpd implements a basic SMTP SmtpServer.
package main //smtpd
...
func ListenAndServe(addr string, handler Handler, appname string, hostname string, authHandler AuthHandler) error {
	srv := &SmtpServer{Addr: addr, Handler: handler, Appname: appname, Hostname: hostname, AuthHandler: authHandler}
...
func ListenAndServeTLS(addr string, certFile string, keyFile string, handler Handler, appname string, hostname string) error {
	srv := &SmtpServer{Addr: addr, Handler: handler, Appname: appname, Hostname: hostname}
...
// LogFunc is a function capable of logging the client-SmtpServer communication.
type LogFunc func(remoteIP, verb, line string)

// SmtpServer is an SMTP SmtpServer.
...
func (srv *SmtpServer) ConfigureTLS(certFile string, keyFile string) error {
...
func (srv *SmtpServer) ConfigureTLSWithPassphrase(
	certFile string,
	keyFile string,
	passphrase string,
) error {
...
func (srv *SmtpServer) Serve(ln net.Listener) error {
	defer ln.Close()
	for {
		conn, err := ln.Accept()
		if err != nil {
...
func (srv *SmtpServer) newSession(conn net.Conn) (s *session) {
	s = &session{
		srv:  srv,
		conn: conn,
...
loop:
	for {
		...
		fmt.Println( "command : " +  verb, args)//jerryprint
		switch verb {
		case "HELO":
			s.remoteName = args
		...
		case "STARTTLS":
			...
			// RFC 3207 specifies that the SmtpServer must discard any prior knowledge obtained from the client.
			s.remoteName = ""
			from = ""
			gotFrom = false
			to = nil
		...
		case "AUTH":
			...
			// Many SmtpServers return 5.7.4 ("Security features not supported") instead.
			switch authType {
			case "PLAIN":
...
//	fmt.Println("makeEHLOResponse... (" +  response + ")")//jerryprint
	return
}
...
//	temp := msg.Header.Get("Content-Disposition") //jerryprint

	dec := new(mime.WordDecoder)
	tmp, err := dec.DecodeHeader(subject)
	if err != nil {
		panic(err)
	}
...
//	fmt.Println(tmp)
	...
	fmt.Printf("Received mail from %s for %s with subject %s\n", from, to[0], tmp)//subject)

	//파일 쓰기
	/*
		err := ioutil.WriteFile(RECEIVEDFILES+"/"+subject+".eml", data, 0)
		if err != nil {
			panic(err)
		}
	*/
	file, err := os.Create(RECEIVEDFILES + "/" + tmp + ".eml")//subject + ".eml")
	if err != nil {
		panic(err)
		return
	}
...
func ocSmtpServer() {
//	fmt.Printf("usage : GoSmtpd -ip=192.168.100.123 -port=25\n")
	var cfg ServerConfigs
	
	cfg = readServerConfigJson("serverconfig.json")

	port := cfg.SmtpPort
	ip := cfg.SmtpIp//"192.168.0.11"//"127.0.0.1"
	//	var emls emlFiles
	RECEIVEDFILES = cfg.SmtpDir
/*
	dir, err := os.Getwd()
	if err != nil {
		log.Fatal(err)
...
*/	
	...
	CreateDirIfNotExist(MYPATH + "/" + RECEIVEDFILES)

	log.Printf("[Smtp Server]ListenAndServe Started! -> Port(%s)", port)

	//	ListenAndServe("127.0.0.1:25", mailHandler, "MySmtpServerApp", "")
//	ListenAndServe(ip+":"+port, mailHandler, "KsmtpSmtpServer", "")
	ListenAndServe(ip+":"+port, mailHandler, "SmtpServer", "", authHandler)
}
