...
var productVersion = "ocGoHash V3.4(Golang)"
...
//			hashedAlgorithm = strings.Trim(strings.Trim(input,"\n"), "\r")
	...
	//			log.Fatal(e)
			...
			cfg.HashedAlgorithm = hashs[hashID-1]
...
func makeHash(cfg Configs) (string, string){
	...
	resultfileName = fmt.Sprintf("%s/%s_%s_%s_%s.txt", RESULT, currentTime.Format("2006-01-02"), receiptNumber, productName, description)
	resultfileName = fmt.Sprintf("%s/%s_%s_%s_%s.txt", RESULT, currentTime.Format("20060102-150405"), receiptNumber, productName, description)
...
//	return resultName_test, newResultName
	return "test", "newtest"
...
//		fmt.Print("\033[u\033[K")
		fmt.Printf("\r%s(%d/%d)", progressBar[progressBarIndex], fileCount, allFiles)
