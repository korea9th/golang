...
//		result := strings.SplitAfterN(scanner.Text(), " ", 3)
		...
		slicedArray := result[1:len(result)-2]
//		fmt.Println(result[0], "\t", slicedArray, "\t", result[len(result)-1])
//		fmt.Println(result[0], "\t", strings.Join(slicedArray, " "), "\t", result[len(result)-1]) 
		fmt.Printf("%s\t%s\t%s\n", result[0], strings.Join(slicedArray, " "), result[len(result)-1]) 
//		fmt.Println(result[0]) // 출력: [apple, banana, cherry,date]
//		fmt.Println(strings.Join(slicedArray, " ")) // 출력: [apple, banana, cherry,date]
//		fmt.Println(result[len(result)-1]) // 출력: [apple, banana, cherry,date]
