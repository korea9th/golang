...
//	e.Cc = []string{"test_cc@example.com"}
	...
	e.AttachFile("downloaded/001.jpg")
//	e.Send("smtp.gmail.com:587", smtp.PlainAuth("", e.From, "password123", "smtp.gmail.com"))
//	err := e.Send("127.0.0.1:25", smtp.PlainAuth("", e.From, "password123", "127.0.0.1"))
	err := e.Send("127.0.0.1:25", nil)
	fmt.Printf("%s\n", err)
...
//	filelist := cfg.SmtpFileList
	...
	fromMail := "JerryBlack <user_red@blacksmith.com>"
	toMail := "user_green@blacksmith.com"

	
	i := 0
	err := filepath.Walk(MYPATH + "/testfiles", func(path string, info os.FileInfo, err error) error {
		if info.IsDir() {
			return nil
		}
		e := NewEmail()
	...
	//	e.Cc = []string{"test_cc@example.com"}
		e.Subject = info.Name()
		e.Text = []byte("Text Body is supported.("+info.Name()+")\n")
		e.HTML = []byte("<h1>Html is supported, too!</h1><br><h2>("+info.Name()+")</h2>\n")
		e.AttachFile(path)
	//	e.Send("smtp.gmail.com:587", smtp.PlainAuth("", e.From, "password123", "smtp.gmail.com"))
	//	err := e.Send("127.0.0.1:25", smtp.PlainAuth("", e.From, "password123", "127.0.0.1"))
		err = e.Send(server, nil)
		if err != nil {
			fmt.Printf("%s\n", err)
		}
