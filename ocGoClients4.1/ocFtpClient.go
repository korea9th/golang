...
func ocFtpClient() {
	...
	port := cfg.FtpPort
	user := cfg.FtpId
	passwd := cfg.FtpPassword
	DOWNLOADED := cfg.FtpDownloadDir
    ...
    if ftp, err = Connect(server); err != nil {
        panic(err)
    }
...
//	for _, filename := range files {
	for i := 0; i <len(files); i++ {
		newfilename := strings.Trim(strings.Trim(files[i], "\r"), "\n")
		newfilename  = strings.Trim(strings.Trim(newfilename, "\n"), "\r")

		_, err = ftp.Retr(newfilename, func(r io.Reader) error {
			out, err := os.Create(DOWNLOADED + "/" + newfilename)
			if err != nil {
				for j := 0; j < len(newfilename); j++ {
					fmt.Printf("%02x", newfilename[j])
				}
				panic(err)
				return err
			}
